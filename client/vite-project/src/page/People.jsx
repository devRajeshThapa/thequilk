...
export default function People() {
  ...
  useEffect(() => {
    const fetchpeople = async () => {
      try {
        const response = axios.get(`http://${IP}:${PORT}/upload/file/people`);
      ...
      <div className="usercontainer">
        ...
        <div className="userdiv">
          {filteredPeople.length > 0 ? (
            peopledata &&
            filteredPeople.map((current, index) => {
              return (
                <div className="personcontainer" key={index}>
                  <img
                    src={`http://${IP}:${PORT}/${current.profile}`}
                    alt="Selected"
                    className="personimage"
                    onClick={(e) => {
                      handleprofile(e, current._id);
                    }}
                  />
            ...
            <div className="nodiv">
              <p className="nocontent">Uff !!! No User</p>
            </div>
          )}
        </div>
      </div>
